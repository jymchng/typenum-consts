...
pub(crate) fn tnconst_impl(lit_integer: LitInteger) -> Result<TokenStream2> {
    match lit_integer {
        LitInteger::UnsignedInteger { lit_integer } => uconst_impl::uconst_impl(lit_integer),
        LitInteger::PositiveInteger { lit_integer } => pconst_impl(lit_integer),
        LitInteger::NegativeInteger { lit_integer } => nconst_impl(lit_integer),
    }
}
